...
import Enemy from "../enemy/enemy1.js";
...
export default class EntityManager {
    ...
    _createNewStuff (tick) {    // Looks like I can manipulate this function to add my new enemy at certain points of the tick.
        ...
        if (tick > 800) createFactor += 0.02; // Adding to createFactor makes new enemy more likely.  
        ...
        if ( /* Math.random() < createFactor && tick < 1200 */ tick < 10) { // createFactor keeps increasing so creation of enemy1 is increased?? Maybe I could switch it to 'greater than' so they are gradually reduced? Or add another clause (what's the word for this? Expression?) to this if statement so they only get created when tick is between two certain figures.
        ...
        if ( /*Math.random() < createFactor && */ tick < 10 ) { 
            new Enemy2(); 
        }
